    ...
    protected void onCreate(Bundle savedInstanceState) {
                    ...
                    public void run() {
                        if(result.getBarcodeFormat().equals(BarcodeFormat.CODE_128)) {
                            if (!String.valueOf(result.getText()).isEmpty()) {
                                ...
                                if (mDatabase.stockCountScanLineDao().isAddToCart(text) != 1) {
                                    mDatabase.stockCountScanLineDao().add(new StockCountScanLine(0, text, DateUtils.currentDateTimeYYYY()));
                                } else {
                                    Toast.makeText(StockCountBatchScanActivity.this, "Scanned Itemcode. Already Exist.", Toast.LENGTH_SHORT).show();
                                }
                            } else {
        ...
        findViewById(R.id.btnAdd).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                List<StockCountScanLine> line = mDatabase.stockCountScanLineDao().getDataList();
                for(StockCountScanLine li:line){
                    if(mDatabase.stockCountLineDao().isAddToCart(li.getItemcode()) == 1){
                        if(mDatabase.stockCountLineDao().getLineStatus(li.getItemcode()).equalsIgnoreCase("A")){
                        ...
                        mDatabase.stockCountLineDao().updateStatus(li.getItemcode(), li.getScanDate());
                    }else{
                ...
                if(alertText.length() != 0){
                    AlertDialog.Builder builder = new AlertDialog.Builder(StockCountBatchScanActivity.this);
...
/*
    @Override
    public void onUpdate(int position, StockCountLine models) {

        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle(models.getItemName()+"");
        final View customLayout = getLayoutInflater().inflate(R.layout.custom_alart_act_qty_layout, null);
        builder.setView(customLayout);
        TextView tvDatd = customLayout.findViewById(R.id.text);
        tvDatd.setText(models.getItemCode());
        EditText etActQty = customLayout.findViewById(R.id.editText);
        EditText etRemarks = customLayout.findViewById(R.id.editMultiText);
        etActQty.setText(models.getActQty()+"");
        etRemarks.setText(models.getRemarks()+"");

        builder.setPositiveButton("Update", (dialog, which) -> {

            Float actQ = Float.valueOf(etActQty.getText().toString());
            String etRe = etRemarks.getText().toString();
            mDatabase.stockCountLineDao().updateBatchInfo(models.getSlno(), actQ, etRe);
            dialog.dismiss();
        });
